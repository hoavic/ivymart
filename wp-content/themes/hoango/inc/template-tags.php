...
if ( ! function_exists( 'hoango_posted_on' ) ) :
	...
	function hoango_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'hoango' ),
...
if ( ! function_exists( 'hoango_posted_by' ) ) :
	...
	function hoango_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'hoango' ),
	...
	function hoango_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'hoango' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'hoango' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'hoango' ),
						array(
							'span' => array(
								'class' => array(),
							),
